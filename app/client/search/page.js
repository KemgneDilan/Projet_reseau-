    ...
    setTimeout(() => {
      ...
      if (includesListings) {
        const filtered = listings.filter((listing) => {
          ...
          if (params?.q) {
            return (listing.title + ' ' + listing.description).toLowerCase().includes(params.q.toLowerCase()) && matchPrice && matchRating
          }
          return matchPrice && matchRating
        }).map(l => ({ type: 'listing', data: l }))
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-3">
            <label className="text-sm font-semibold">Je cherche:</label>
            <select value={searchTarget} onChange={e => setSearchTarget(e.target.value)} className="border px-3 py-2 rounded-lg">
              <option value="listings">Chambres</option>
