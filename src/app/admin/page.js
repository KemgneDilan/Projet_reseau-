...
export default function AdminDashboard() {
  ...
  const handleRejectKYC = (requestId, userId) => {
    const updatedUsers = usersList.map(u => {
      if (u.id === userId) {
  ...
  const kpis = [
    { title: 'Utilisateurs Actifs', value: usersList.length.toString(), icon: Users, color: 'text-blue-500', bg: 'bg-blue-50' },
    { title: 'Logements Publiés', value: listings.length.toString(), icon: Building, color: 'text-green-500', bg: 'bg-green-50' },
    { title: 'Vérifications KYC en attente', value: pendingRequests.length.toString(), icon: ShieldCheck, color: 'text-purple-500', bg: 'bg-purple-50' },
    { title: 'Réservations Globales', value: contracts.length.toString(), icon: FileText, color: 'text-terracotta-500', bg: 'bg-terracotta-50' },
  ]
  ...
  const getEntityName = (id, type) => {
    return listings.find((l) => l.id === id)?.title || houses.find((h) => h.id === id)?.title || id
  }
                ...
                <table className="w-full text-left">
                  <thead className="bg-charcoal-50 dark:bg-charcoal-950/20 text-xs font-semibold text-charcoal-500 dark:text-charcoal-400 uppercase">
                    <tr>
                      ...
                      <th className="px-6 py-4">Rôle Externe</th>
                      ...
                      return (
                        <React.Fragment key={u.id}>
                          <tr className={`hover:bg-charcoal-50/50 dark:hover:bg-charcoal-900/50 transition-colors ${isExpanded ? 'bg-charcoal-50/80 dark:bg-charcoal-900/80' : ''}`}>
                            ...
                            <td className="px-6 py-4 text-charcoal-500 dark:text-charcoal-400">{u.phone || 'Non renseigné'}</td>
                            <td className="px-6 py-4 text-right">
                              ...
                              >
                                {userContracts.length} réservation{userContracts.length !== 1 ? 's' : ''}
                                        ...
                                        <div key={c.id} className="flex justify-between items-center bg-white dark:bg-charcoal-950 p-3 rounded-lg border border-charcoal-100 dark:border-charcoal-800">
                                          <div>
                                            ...
                                            <p className="text-xs text-charcoal-500 dark:text-charcoal-400">
                                              {c.date} • {u.role === 'client' ? `Hôte: ${getProviderName(c.providerId)}` : `Voyageur: ${getClientName(c.clientId)}`}
                                            </p>
                                          </div>
                    ...
                    {houses.map(h => {
                      ...
                      return (
                        <React.Fragment key={h.id}>
                          ...
                          {isExpanded && (
                            <tr>
                              <td colSpan={3} className="p-0 border-b border-charcoal-100 dark:border-charcoal-800">
              ...
              <CardContent className="p-0 overflow-x-auto">
                {kycRequests.length === 0 ? (
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-charcoal-50 dark:bg-charcoal-950/20 text-xs font-semibold text-charcoal-500 dark:text-charcoal-400 uppercase">
                      <tr>
                        <th className="px-6 py-4">Voyageur (Username)</th>
