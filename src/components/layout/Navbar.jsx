...
import { Home, Search, MessageSquare, Menu, User, LogOut, Heart, Calendar, Settings, ChevronDown, Repeat2 } from "lucide-react"
...
export function Navbar() {
  ...
  const userMenuItems = [
    { label: t('nav_profile'), icon: User, href: `/${currentMode}` },
    { label: t('nav_reservations'), icon: Calendar, href: `/${currentMode}` },
  ...
  // Toggle is only shown to users who have passed KYC verification
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            {!isLoading && user ? (
              <>
                ...
                <div className="flex items-center gap-2 relative">
                  ...
                  {/* Mobile Menu Button - placed just BEFORE the profile avatar */}
                  <Button
                    variant="ghost"
                    size="icon"
                    className="sm:hidden"
                      ...
                      >
                        <div className="px-4 py-3 border-b border-charcoal-200">
                          ...
                          <p className="text-xs text-charcoal-500 mt-0.5">
                            Mode actif : <span className={`font-semibold ${currentMode === 'host' ? 'text-emerald-600' : 'text-terracotta-600'}`}>
                              {currentMode === 'host' ? '🏠 Hôte' : '✈️ Voyageur'}
                            </span>
                          </p>
                        </div>

                        {/* Mode Toggle — only for KYC-validated users */}
                        {canToggleHostMode && (
                          <div className="px-3 py-3 border-b border-charcoal-200">
                            <button
        ...
        {isMenuOpen && (
          ...
          >
            ...
            {user && (
              <div className="pt-4 border-t border-charcoal-200 space-y-2">
                {/* Mobile mode toggle — only for KYC-validated users */}
                {canToggleHostMode && (
                  <button
                    onClick={handleModeToggle}
